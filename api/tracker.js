...
// ============================================================
// ===== MOTOR ML — LOGISTIC REGRESSION v2.0 =====
// ============================================================

const MODELO_BASE_BIAS = -1.2;
...
// Resolve nome da aba para nome do modelo
function resolverSite(siteName) {
    const tab = SITE_TABS[siteName] || siteName || 'Site Principal';
    return MODELOS_SITE[tab] ? tab : 'Site Principal';
}
...
function extrairFeatures(data, geo) {
    ...
    try {
        ...
        if (sv.length > 2 && sv.includes(':')) {
            sv.split(',').forEach(item => {
                ...
                if (colonIdx > 0) {
                    sections[item.substring(0, colonIdx).trim().toLowerCase()] = 1;
                }
            });
        } else {
            // Tenta como JSON
    ...
    const visita      = data.first_visit; // booleano do tracker
    const botao       = String(data.whatsapp_button || '').toLowerCase();
    const bateria     = parseFloat(data.battery_level) || 1.0;
    const regiao      = ['sarandi','paicandu','mandaguari','marialva','astorga','iguaracu','paicandu'];
    ...
    return {
        ...
        // Visita (booleano do tracker: true = primeira visita)
        primeira_visita: visita === true || visita === 'true' ? 1 : 0,
        retorno:         visita === false || visita === 'false' ? 1 : 0,
...
function calcularScoreML(data, geo, siteName) {
    try {
        const siteKey = resolverSite(siteName);
        const modelo  = MODELOS_SITE[siteKey];
        const features = extrairFeatures(data, geo);

        let z = MODELO_BASE_BIAS + modelo.bias;
        const contribuicoes = [];

        for (const [nome, valor] of Object.entries(features)) {
            const peso   = modelo.weights[nome] || 0;
        ...
        return { score, categoria, urgencia, cor, topRazoes, modelo: 'v2.0-' + siteKey.toLowerCase().replace(/\s+/g,'_') };

    } catch(err) {
        console.error('ML error:', err.message);
        return { score: 50, categoria: 'ERRO', urgencia: 'Verificar manualmente', cor: '#666', topRazoes: ['erro'], modelo: 'erro' };
    }
}

// ============================================================
// ===== EMAIL HTML (versão unificada com Score ML) =====
// ============================================================
    ...
    try {
        ...
        if (timeline.length > 0) {
            ...
            <div style="background:#f8fafc;padding:12px 16px;border-radius:8px;font-size:12px;font-family:monospace;color:#334155;line-height:1.8;">
                ${timeline.map(e => `<span style="color:#94a3b8;">${e.t}s</span> &rarr; <strong>${e.type}</strong>${e.detail ? ' (' + e.detail + ')' : ''}`).join('<br>')}
            </div>`;
        }
    } catch(e) {}
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.10);max-width:600px;">
  ...
  <!-- HEADER -->
  <tr>
    <td style="background:#25d366;padding:18px 24px;">
  ...
  <tr><td style="padding:20px 24px 0;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:${corFundo};border:2px solid ${ml.cor};border-radius:12px;margin-bottom:16px;">
      <tr><td style="padding:18px;text-align:center;">
        <p style="margin:0;font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:1px;">Score ML | Modelo ${ml.modelo}</p>
        <p style="margin:6px 0 0;font-size:60px;font-weight:900;color:${ml.cor};line-height:1;">${ml.score}%</p>
        <p style="margin:6px 0 0;font-size:15px;font-weight:700;color:${ml.cor};">[${ml.categoria}]</p>
    ...
    <!-- INFO BÁSICA -->
    <div style="background:#f0fdf4;border-left:4px solid #25d366;padding:14px 16px;border-radius:0 8px 8px 0;margin-bottom:4px;">
      <div style="font-size:13px;color:#333;line-height:1.8;">
        <strong>Session ID:</strong> ${data.session_id || '-'}<br>
        <strong>Pagina:</strong> ${data.page_url || '-'}<br>
        <strong>Botao:</strong> ${data.whatsapp_button || '-'}
      </div>
    </div>

    ${alerts.length > 0 ? `<div style="background:#fef2f2;border:1px solid #fca5a5;border-radius:8px;padding:10px 14px;margin:12px 0;font-size:13px;color:#991b1b;">${alerts.join(' | ')}</div>` : ''}
    ...
    ${(geo.lat !== '-' && geo.lon !== '-') ? `<div style="margin:6px 0 4px;"><a href="https://www.google.com/maps?q=${geo.lat},${geo.lon}" style="font-size:12px;color:#1e3a8a;font-weight:600;">Ver no Google Maps</a></div>` : ''}
    ...
    <h3 style="color:#1e3a8a;font-size:13px;text-transform:uppercase;border-bottom:2px solid #e2e8f0;padding-bottom:6px;margin:22px 0 10px;">Parametros UTM</h3>
    <div style="background:#f8fafc;padding:12px 16px;border-radius:8px;font-size:12px;font-family:monospace;color:#334155;line-height:1.8;">
      ...
      <tr><td style="padding:16px;text-align:center;">
        ...
        <table cellpadding="0" cellspacing="0" style="margin:0 auto;">
          <tr><td style="background:#f59e0b;border-radius:8px;padding:10px 24px;">
            <a href="${formLink}" style="color:#fff;font-size:13px;font-weight:700;text-decoration:none;">Preencher Feedback</a>
          </td></tr>
        </table>
      </td></tr>
    </table>
  ...
  <!-- FOOTER -->
  <tr>
    <td style="background:#f8fafc;border-top:1px solid #e2e8f0;padding:12px 24px;text-align:center;">
      <p style="margin:0;font-size:10px;color:#94a3b8;">Maringa Vasculares - Sistema ML ${ml.modelo} | tracker v4</p>
    </td>
  </tr>
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        // 1. Sempre grava no Sheets (igual ao original)
        const sheetsOk = await writeToSheets(data, geo, ip);

        // 2. Se clicou WhatsApp → calcular ML + enviar email
        let emailOk = true;
        if (data.send_email && data.event_type === 'whatsapp_click') {
            try {
                // Calcular Score ML
                const ml = calcularScoreML(data, geo, data.site_name);
